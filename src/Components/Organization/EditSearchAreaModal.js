...
const EditSearchAreaModal = ({
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const { data } = await axios.put(
        `/api/v1/searchareas/update/${searchArea._id}`,
        formData
      );
  ...
  useEffect(() => {
    ...
    if (searchArea) {
      reset({
        dogHandlerName: searchArea.name || "",
        dogHandlerEmail: searchArea.email || "",
        reporterName: searchArea.recipient?.name || "",
        reporterEmail: searchArea.recipient?.email || "",
        reporterNumber: searchArea.recipient?.phone || "",
        instructions: searchArea.instructions || "",
      });
    }
  }, [searchArea, reset]);
  return (
...
export default EditSearchAreaModal;
